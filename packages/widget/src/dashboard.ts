...
import { ALL_AUDIENCES, captionsFor } from "./decisions.js";
 ...
 *  categories), or "" when none are addressed to the given audiences. Shared by the dashboard grid
 *  and the single-chart path so a chart's captions reach the human on every surface. */
export function renderChartNotes(spec: ChartSpec, audiences: readonly DecisionAudience[] = ALL_AUDIENCES): string {
  const lines = captionsFor(spec, audiences);
  return lines.length ? `<div class="cell-notes">${esc(lines.join(" "))}</div>` : "";
}

/** A cell's hard failure. Not an advisory, so it renders whatever the audience filter is. */
function renderItemError(error: unknown): string {
  return typeof error === "string" && error ? `<div class="cell-error">${esc(error)}</div>` : "";
}
...
): { kind: string; inner: string } {
  const error = renderItemError((item as { error?: unknown }).error);
  if ("spec" in item) {
    // main.ts paints the chart into #cell-<i>; a per-cell caption (capped categories, a folded
    // tail) renders here as a sibling so the human sees it, mirroring dash-notes.
    const note = renderChartNotes(item.spec, audiences);
    return { kind: "chart", inner: `<div class="cell-chart" id="cell-${index}"></div>${error}${note}` };
  }
  if (item.type === "kpi") return { kind: "kpi", inner: renderKpi(item) + error };
...
function renderItem(
  ...
  const { kind, inner } = itemBody(item, index, audiences);
...
export function renderSingleItem(item: DashboardItem, opts: CaptionOptions = {}): string {
  const { kind, inner } = itemBody(item, 0, audiencesFrom(opts));
  return `<div class="solo ${kind}">${inner}</div>`;
}

/** Which decisions this surface captions. */
export interface CaptionOptions {
  /** Decision audiences to draw. Defaults to all of them; embed mode narrows it. */
  audiences?: readonly DecisionAudience[];
  /** Deprecated alias: `false` draws no captions at all. `audiences` wins when both are given. */
  notes?: boolean;
}

function audiencesFrom(opts: CaptionOptions): readonly DecisionAudience[] {
  if (opts.audiences) return opts.audiences;
  return opts.notes === false ? [] : ALL_AUDIENCES;
}

export interface DashboardShellOptions extends CaptionOptions {
...
export function renderDashboardShell(spec: DashboardSpec, opts: DashboardShellOptions = {}): string {
  const audiences = audiencesFrom(opts);
  const columns = clampCols(spec.columns);
  const title = spec.title && opts.titled !== false ? `<div class="dash-title">${esc(spec.title)}</div>` : "";
  const cells = spec.items.map((item, i) => renderItem(item, i, columns, audiences)).join("");
  const grid = `<div class="grid" style="--cols:${columns}">${cells}</div>`;
  const lines = captionsFor(spec, audiences);
  const notes = lines.length ? `<div class="dash-notes">${esc(lines.join(" "))}</div>` : "";
